...
Group projects receive a **team grade, but individual grades can be adjusted** for contribution. We look at each member's git commit history and AI-interaction log (`history.md`), and after each group project every member completes a short **anonymous peer check** on how the work was shared. A visibly one-sided contribution — or a teammate whom peers consistently flag as absent — can move that individual's grade up or down relative to the team's. If tensions or an unfair split are developing, tell us **during** the project (office hours or email), not after grades are out — we would much rather help you rebalance the work than adjust a grade afterward.
